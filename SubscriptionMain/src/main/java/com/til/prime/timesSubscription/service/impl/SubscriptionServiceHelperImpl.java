...
public class SubscriptionServiceHelperImpl implements SubscriptionServiceHelper {
    ...
    public UserSubscriptionModel generateInitPurchaseUserSubscription(UserDTO userDTO, ChannelEnum channel, PlatformEnum platform, PlanTypeEnum planType, Long durationDays, SubscriptionVariantModel variantModel, UserSubscriptionModel lastUserSubscription, UserModel userModel, BigDecimal price, boolean crmRequest, boolean free){
        ...
        userSubscriptionModel.setStatus(StatusHelper.getStatusForUserSubscription(userSubscriptionModel, date));
        ...
        if(price.compareTo(BigDecimal.ZERO)<=0 || (crmRequest&&free)){
            ...
            userSubscriptionModel.setPlanStatus(PlanStatusHelper.getPlanStatus(userSubscriptionModel.getStatus(), planType, variantModel.getPrice(), lastUserSubscription,  false));
    ...
    public UserSubscriptionModel updateSubmitPurchaseUserSubscription(SubmitPurchaseRequest request, UserSubscriptionModel userSubscriptionModel, UserSubscriptionModel lastUserSubscription) {
        ...
        userSubscriptionModel.setPlanStatus(PlanStatusHelper.getPlanStatus(userSubscriptionModel.getStatus(), variantModel.getPlanType(), userSubscriptionModel.getPrice(), lastUserSubscription,  request.isAutoRenewalJob()));
    ...
    public SubscriptionPlanVariantResponse preparePlanDetailsByVariantResponse(SubscriptionPlanVariantResponse response, SubscriptionVariantModel model, ValidationResponse validationResponse) {
        if(validationResponse.isValid()){
            ...
            response.setVariant(ModelToDTOConvertorUtil.getSubscriptionVariantDTO(model));
    ...
    public UserSubscriptionModel extendSubscription(UserSubscriptionModel userSubscriptionModel, Long extensionDays) {
        ...
        userSubscriptionModel.setStatus(StatusHelper.getStatusForUserSubscription(userSubscriptionModel, null));
        userSubscriptionModel.setStatusDate(new Date());
        userSubscriptionModel.setStatusPublished(false);
    ...
    private final boolean communicateSSO(UserSubscriptionModel userSubscriptionModel){
        ...
        while (retryCount>0){
            try{
                ...
                headers.put(GlobalConstants.MOBILE, userSubscriptionModel.getUser().getMobile());
