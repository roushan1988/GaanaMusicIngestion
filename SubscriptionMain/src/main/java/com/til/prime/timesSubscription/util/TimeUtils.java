...
public class TimeUtils {
	...
	public static final Date getPrevDate(Date date){
		Calendar calendar = Calendar.getInstance();
		calendar.setTime(date);
		calendar.add(Calendar.DATE, -1);
		return calendar.getTime();
	...
	public static final Date addDaysFromNow(int numberOfDays){
		Calendar calendar = Calendar.getInstance();
    ...
    public static final long getDaysBetweenDates(Date earlyDate, Date nextDate) {
        long diffs = nextDate.getTime() - earlyDate.getTime();
        return TimeUnit.DAYS.convert(diffs, TimeUnit.MILLISECONDS);
    }

	public static final String getDaysBreakText(long numOfDays) {
		if (numOfDays < 365) {
			return format("{0} {1}", numOfDays, numOfDays < 2 ? "day" : "days");
		}

		int year = (int) numOfDays / 365;
		return format("{0} {1}", year, year < 2 ? "year" : "years");
	}
