...
import helper.PlanStatusHelper;
...
public class SubscriptionServiceImpl implements SubscriptionService {
    ...
    public PlanListResponse getAllPlans(PlanListRequest request) {
        ...
        ValuePropDTO valuePropDTO = PlanStatusHelper.getPlanTexts(null, PlanStatusEnum.INIT, null, null, channel, null, daysForPayment);
        ...
        if (validationResponse.isValid()) {
            if (request.getUser() != null) {
                ...
                try {
                    ...
                    valuePropDTO = PlanStatusHelper.getPlanTexts(valuePropDTO, getPlanStatusEnum(subscriptionStatusDTO.getPlanStatus()), subscriptionStatusDTO.getEndDate(), subscriptionStatusDTO.getLastEndDate(), channel, format("₹{0}", price), daysForPayment);
    ...
    public CancelSubscriptionResponse cancelSubscription(CancelSubscriptionRequest request, boolean serverRequest) {
        ...
        if (validationResponse.isValid()) {
            ...
            if (refundResponse.isSuccess()) {
                ...
                userSubscriptionModel.setPlanStatus(PlanStatusHelper.getPlanStatus(userSubscriptionModel.getStatus(), userSubscriptionModel.getSubscriptionVariant().getPlanType(), userSubscriptionModel.getPrice(), null,  false));
...
//                    }
    ...
    public SubscriptionExpired expireSubscription(Long subsId) throws Exception {
        ...
        if(userSubscriptionModel1!=null){
            ...
            userSubscriptionModel1.setPlanStatus(PlanStatusHelper.getPlanStatus(userSubscriptionModel1.getStatus(), userSubscriptionModel1.getSubscriptionVariant().getPlanType(), userSubscriptionModel1.getPrice(), userSubscriptionModel, false));
...
//            }
    ...
    private UserSubscriptionModel expireUserSubscription(UserSubscriptionModel userSubscriptionModel) {
        ...
        userSubscriptionModel.setStatusDate(new Date());
        userSubscriptionModel.setPlanStatus(PlanStatusHelper.getPlanStatus(StatusEnum.EXPIRED, userSubscriptionModel.getSubscriptionVariant().getPlanType(), userSubscriptionModel.getPrice(), null, false));
        userSubscriptionModel.setSsoCommunicated(false);
        userSubscriptionModel.setStatusPublished(false);
