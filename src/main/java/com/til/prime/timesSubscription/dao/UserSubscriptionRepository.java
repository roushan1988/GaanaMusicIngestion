...
public interface UserSubscriptionRepository extends GenericJpaRepository<UserSubscriptionModel, Long> {
    ...
    @Query(value="select us from UserSubscriptionModel us where us.statusPublished = false and us.id in (select max(id) from UserSubscriptionModel us1 where us1.orderCompleted = true and us1.deleted = false and us1.user.deleted = false group by us1.user)")
    List<UserSubscriptionModel> findUserSubscriptionsForStatusPublish(Pageable pageable);
    ...
    UserSubscriptionModel findFirstByUserMobileAndUserDeletedFalseAndDeletedFalseAndOrderCompletedTrueAndEndDateAfterAndStatusIn(String mobile, Date date, Collection<StatusEnum> statusEnums);
    ...
    List<UserSubscriptionModel> findByStatusAndDeletedFalseAndOrderIdNotNull(StatusEnum status);
    List<UserSubscriptionModel> findByStatusAndCreatedBetweenAndDeletedFalseAndOrderIdNotNull(StatusEnum status, Date date1, Date date2);
    List<UserSubscriptionModel> findByCreatedBetweenAndDeletedFalseAndOrderIdNotNull(Date date1, Date date2);
    List<UserSubscriptionModel> findByOrderIdAndDeletedFalseAndOrderIdNotNull(String orderId);
